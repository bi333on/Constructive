...
export function useAutosave() {
  ...
  useEffect(() => {
    ...
    // Немедленное сохранение (сбрасывает отложенный таймер).
    const saveNow = async () => {
      if (timer.current) {
    ...
    // При сворачивании/закрытии вкладки сохраняем сразу, чтобы не потерять
    // правки, внесённые в последние 5 секунд.
    const flushIfDirty = () => {
      if (useSaveStore.getState().status === "dirty") saveNow();
    };
    const onVisibility = () => {
